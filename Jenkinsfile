...
pipeline {
	...
	parameters {
	  choice(name: 'ExecEnvironment', choices: ['qa'], description: 'Pick an environment to run scripts')
        string(name: 'ModuleName', defaultValue: 'ClinicalSalesDiscountTests.xml', description: 'Pick scripts by features')
        ...
        booleanParam(name: 'setTrace', defaultValue: false, description: 'Start playwright trace for analysis')
        ...
        string(name: 'SalesForceLoginID', defaultValue: 'automationuser@consensus.com.ccsi-sf.qa', description: 'Your SalesForce CCSI LoginID, not the e-mail address')
        password(name: 'SalesForcePassword', defaultValue: 'Automation@123', description: 'Your SalesForce CCSI Password')
	}
                    ...
                    if (isUnix())
                     {
                        sh """
                            if [ -d "${videoDir}" ]; then
                                find "${videoDir}" -type f -mtime +${daysOld} -exec rm -f {} +
                            fi
                        """
                        } else
                         {
                        bat """
                            if exist "${videoDir}" (
                                powershell -Command "Get-ChildItem -Path '${videoDir}' -Recurse -File | Where-Object { \$_.LastWriteTime -lt (Get-Date).AddDays(-${daysOld}) } | Remove-Item -Force"
                            )
                        """
                            }
